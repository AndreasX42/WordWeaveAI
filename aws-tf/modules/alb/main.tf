# Security Group for ALB - Only allows HTTP/HTTPS from internet
resource "aws_security_group" "alb" {
  ...
  # Allow all outbound traffic by default, will be restricted by specific rules
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
...
resource "aws_security_group" "ecs_tasks" {
  ...
  vpc_id      = var.vpc_id
  description = "Security group for ECS tasks - Only accessible from ALB"
...
# Rules to connect ALB and ECS Tasks
...
resource "aws_security_group_rule" "ecs_ingress_from_alb_backend" {
  type                     = "ingress"
  from_port                = 8080
  to_port                  = 8080
  protocol                 = "tcp"
