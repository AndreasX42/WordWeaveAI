...
func TestSearchHandler_GetMediaByRef(t *testing.T) {
	searchHandler, _, mediaRepo := setupSearchHandler()

	t.Run("successful media retrieval", func(t *testing.T) {
		gin.SetMode(gin.TestMode)
		router := gin.New()
		router.GET("/media/:mediaRef", searchHandler.GetMediaByRef)

		// Make request
		req := httptest.NewRequest(http.MethodGet, "/media/hello_media_ref", nil)
		recorder := httptest.NewRecorder()
		router.ServeHTTP(recorder, req)
	...
	t.Run("missing mediaRef parameter", func(t *testing.T) {
		gin.SetMode(gin.TestMode)
		router := gin.New()
		router.GET("/media/:mediaRef", searchHandler.GetMediaByRef)

		// Make request without mediaRef
		req := httptest.NewRequest(http.MethodGet, "/media/", nil)
		recorder := httptest.NewRecorder()
		router.ServeHTTP(recorder, req)

		// This will result in 404 due to route mismatch, but let's test empty parameter case
		gin.SetMode(gin.TestMode)
		router2 := gin.New()
		router2.GET("/media/", searchHandler.GetMediaByRef)
		...
		assert.Equal(t, "mediaRef is required", response["message"])
	})
	...
	t.Run("media not found", func(t *testing.T) {
		...
		// Make request for non-existent media
		req := httptest.NewRequest(http.MethodGet, "/media/nonexistent_media_ref", nil)
		recorder := httptest.NewRecorder()
		router.ServeHTTP(recorder, req)
		...
		assert.Contains(t, response["details"].(map[string]any)["error"], "media not found")
	})

	t.Run("media service error", func(t *testing.T) {
		// Configure media repo to return error
		mediaRepo.SetError(true, "media service unavailable")
		...
		assert.Equal(t, "Media not found", response["message"])
		assert.Contains(t, response["details"].(map[string]any)["error"], "media service unavailable")

		// Reset error state
		mediaRepo.SetError(false, "")
	})
}
