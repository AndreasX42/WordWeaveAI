...
class WordCategorization(BaseModel):
    source_definition: list[str] = Field(..., min_items=1, max_items=3, description="Definitions of the word its native language")
    source_part_of_speech: PartOfSpeech = Field(..., description="Part of speech of the word")

@tool
async def get_classification(source_word: str, source_language: Language) -> WordCategorization:
