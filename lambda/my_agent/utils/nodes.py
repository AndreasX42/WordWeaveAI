...
async def node_get_classification(state: VocabState) -> VocabState:
    ...
    response = await get_classification.ainvoke(input={"source_word": state.source_word, "source_language": state.source_language})
    
    # print(f"Categorization: {response}")
    
    return {"source_definition": response.source_definition, "source_part_of_speech": response.source_part_of_speech}

async def node_get_translation(state: VocabState) -> VocabState:
    """Translate the word to the target language."""
    response = await get_translation.ainvoke(input={
    ...
    return {"target_word": response.target_word, "target_part_of_speech": response.target_part_of_speech}

async def node_get_synonyms(state: VocabState) -> VocabState:
    """Fetch synonyms for the word."""
    response = await get_synonyms.ainvoke(input={"target_word": state.target_word, "target_language": state.
    target_language, "target_part_of_speech": state.target_part_of_speech})
...
async def node_get_syllables(state: VocabState) -> VocabState:
    ...
    response = await get_syllables.ainvoke(input={
...
async def node_get_pronunciation(state: VocabState) -> VocabState:
    ...
    result = await get_pronunciation.ainvoke(input={
        "target_word": state.target_word, 
...
async def node_get_media(state: VocabState) -> VocabState:
    ...
    response = await get_media.ainvoke(input={
        "source_word": state.source_word,
        "target_word": state.target_word,
        "source_language": state.source_language,
        "target_language": state.target_language
...
async def node_get_examples(state: VocabState) -> VocabState:
    ...
    response = await get_examples.ainvoke(input={
        "source_word": state.source_word,
        "target_word": state.target_word,
        "source_language": state.source_language,
        "target_language": state.target_language
...
async def node_get_conjugation(state: VocabState) -> VocabState:
    ...
    response = await get_conjugation.ainvoke(input={
        "target_word": state.target_word,
        "target_language": state.target_language,
